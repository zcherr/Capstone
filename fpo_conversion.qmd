...
orig_data <- "C:/GitHub/FPO_capstone/output_tag"

subject_folders <- list.dirs(path = orig_data, 
                             recursive = FALSE)
participant_rows <- data.frame(subj = integer(), 
                               row_count = integer())
fpo_df <- data.frame()
...
for (folder in subject_folders) {
  ...
  for (file in file_list) {
    ...
    # Add a new column 'subj' with the subject number
    df$subj <- subj_number
    df$subj <- factor(df$subj, levels = 3:10)

    # Get the number of rows (used for row count confirmation)
    row_count <- nrow(df)
    participant_rows <- rbind(participant_rows, 
                              data.frame(subj = subj_number, 
                                         row_count = row_count))
...
fpo_df |> 
  group_by(subj) |>    
  summarise(new_rows = n()) |> 
  ungroup() |> 
  merge(participant_rows, by = "subj", all = TRUE) |> 
  rename(original_rows = row_count) |> 
  print() 
```
...
```{r}
output_path <- "visualization_files/subj_data/subj_matrix" 
...
folder_path <- "visualization_files/subj_data/subj_matrix" 
...
```{r}
output_dir <- "visualization_files/subj_data/cluster_data"
